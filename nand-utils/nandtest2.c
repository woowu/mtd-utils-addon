...
static NORETURN void usage(int status)
{
    fprintf(status ? stderr : stdout,
        ...
        "  - MTD API error when doing the operation.\n"
        "\n"
        "Read error means:\n"
        "  - MTD API returns error when doing the operation.\n"
        "  - ECC failed number increased from inside the MTD layer. The\n"
        "    API returned code will still be success in this case.\n" 
        "  - All above not happen, but contents comparison between written\n"
        "    copy and the read back copy mismatched.\n"
        "\n"
        "One time of an erase/programming error will lead to marking of bad\n"
        "block immediately if the '-m' option was provided, but only after\n"
        "'times' of contentious read error can lead to marking of bad block.\n"
        "\n"
